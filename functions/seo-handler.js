...
export async function onRequest(context) {
  ...
  // 工具 SEO 配置（与 generate-static.js 中的配置保持一致）
  const seoConfig = {
    '/translate': {
    ...
    '/json-formatter': {
      title: 'JSON格式化 - 在线美化校验压缩工具 | 三八零零',
      description: '免费在线JSON格式化工具，支持JSON数据格式化、校验、压缩，树形结构展示，开发者必备工具。',
      keywords: 'JSON格式化,JSON校验,JSON压缩,JSON解析,JSON在线工具,开发者工具,JSON编辑器',
      h1: 'JSON 格式化工具'
    },
    '/xml-formatter': {
      title: 'XML格式化 - 在线美化校验工具 | 三八零零',
    ...
    '/qrcode-generator': {
      title: '二维码生成器 - 免费制作自定义二维码 | 三八零零',
      description: '免费在线二维码生成器，支持自定义颜色、尺寸、容错级别，一键下载PNG图片。',
      keywords: '二维码生成,二维码制作,QR码生成,在线二维码,二维码工具,网址二维码',
      h1: '二维码生成器'
    },
    '/chinese-converter': {
  ...
  if (seo && isCrawler) {
    ...
    const currentUrl = `https://tools.3800ai.com${path}/`;
    ...
    html = html.replace(/<meta\s+name="seo:title"\s+content="[^"]*"\s*\/?>/i, `<meta name="seo:title" content="${seo.title}">`);
    html = html.replace(/<meta\s+name="seo:description"\s+content="[^"]*"\s*\/?>/i, `<meta name="seo:description" content="${seo.description}">`);
    html = html.replace(/<meta\s+name="seo:keywords"\s+content="[^"]*"\s*\/?>/i, `<meta name="seo:keywords" content="${seo.keywords}">`);
    html = html.replace(/<meta\s+name="seo:canonical"\s+content="[^"]*"\s*\/?>/i, `<meta name="seo:canonical" content="${currentUrl}">`);
    ...
    html = html.replace(/<meta\s+property="og:url"\s+content="[^"]*"\s*\/?>/i, `<meta property="og:url" content="${currentUrl}">`);
    ...
    html = html.replace(/<meta\s+name="twitter:url"\s+content="[^"]*"\s*\/?>/i, `<meta name="twitter:url" content="${currentUrl}">`);
