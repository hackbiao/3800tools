...
type PromptCategory = 'drawing' | 'video' | 'article' | 'ppt';
...
const MODIFIERS: Record<PromptCategory, ModifierGroup[]> = {
    drawing: [
        {
            id: 'quality',
            name: '画面质量',
        ...
        },
        {
            ...
            options: [
                ...
                { value: 'intricate', label: '精细复杂' },
                { value: 'sharp focus', label: '清晰对焦' },
                { value: 'bokeh background', label: '背景虚化' },
                { value: 'detailed texture', label: '细节纹理' },
            ]
        },
    ],
    video: [
        {
            ...
            name: '视频质量',
            options: [
                { value: '4K resolution', label: '4K分辨率' },
                { value: '8K resolution', label: '8K分辨率' },
                { value: 'cinematic quality', label: '电影级质量' },
        ...
        },
        {
            ...
            name: '镜头运动',
            options: [
                { value: 'smooth pan', label: '平移镜头' },
                { value: 'zoom in', label: '推进' },
                { value: 'zoom out', label: '拉远' },
                { value: 'tracking shot', label: '跟踪拍摄' },
                { value: 'static shot', label: '固定镜头' },
                { value: 'drone shot', label: '无人机拍摄' },
                { value: 'handheld', label: '手持拍摄' },
                { value: '360 rotation', label: '360度旋转' },
            ]
        },
        {
            id: 'pace',
        ...
        },
        {
            ...
            name: '色彩处理',
            options: [
                { value: 'color grading', label: '调色处理' },
                { value: 'warm tones', label: '暖色调' },
                { value: 'cool tones', label: '冷色调' },
                { value: 'desaturated', label: '低饱和度' },
    ...
    article: [
        {
            ...
            options: [
                ...
                { value: 'tutorial guide', label: '教程指南' },
                { value: 'product description', label: '产品描述' },
                { value: 'social media post', label: '社交媒体' },
                { value: 'technical article', label: '技术文章' },
                { value: 'news article', label: '新闻报道' },
                { value: 'review', label: '评测评论' },
            ]
        },
        {
            id: 'tone',
            name: '语气风格',
            options: [
                { value: 'professional tone', label: '专业正式' },
                { value: 'casual and friendly', label: '轻松友好' },
        ...
        },
        {
            ...
            name: '目标读者',
            options: [
                { value: 'for beginners', label: '初学者' },
                { value: 'for professionals', label: '专业人士' },
                { value: 'for general audience', label: '普通读者' },
                { value: 'for experts', label: '专家级' },
                { value: 'for young adults', label: '年轻人' },
            ]
        },
        {
            id: 'length',
            name: '文章长度',
            options: [
                { value: 'short (around 200 words)', label: '短文(200字)' },
                { value: 'medium (around 500 words)', label: '中等(500字)' },
                { value: 'long (around 1000 words)', label: '长文(1000字)' },
                { value: 'comprehensive (1500+ words)', label: '详尽(1500+字)' },
            ]
        },
        {
            id: 'structure',
            name: '内容结构',
            options: [
                { value: 'with practical examples', label: '含实例' },
                { value: 'step-by-step format', label: '分步说明' },
                { value: 'bullet points', label: '要点列表' },
                { value: 'storytelling approach', label: '故事叙述' },
                { value: 'Q&A format', label: '问答形式' },
                { value: 'with data and statistics', label: '含数据统计' },
            ]
        },
        {
            id: 'feature',
            name: '特殊要求',
            options: [
                { value: 'SEO optimized', label: 'SEO优化' },
                { value: 'include call-to-action', label: '含行动号召' },
                { value: 'with headings and subheadings', label: '含标题层级' },
                { value: 'engaging introduction', label: '引人入胜的开头' },
                { value: 'strong conclusion', label: '有力的结论' },
            ]
        },
    ],
    ppt: [
        {
            ...
            name: '演示风格',
            options: [
                { value: 'professional business', label: '商务专业' },
                { value: 'modern minimalist', label: '现代极简' },
                { value: 'creative and colorful', label: '创意多彩' },
                { value: 'corporate formal', label: '企业正式' },
                { value: 'educational', label: '教育风格' },
                { value: 'tech and futuristic', label: '科技未来' },
                { value: 'elegant and luxury', label: '优雅奢华' },
            ]
        },
        {
            id: 'layout',
            name: '页面布局',
            options: [
                { value: 'clean layout', label: '简洁布局' },
                { value: 'grid-based', label: '网格布局' },
                { value: 'full-screen image', label: '全屏图片' },
                { value: 'split screen', label: '分屏布局' },
                { value: 'timeline layout', label: '时间线' },
                { value: 'infographic style', label: '信息图表' },
            ]
        },
        {
            id: 'color',
            name: '配色方案',
            options: [
                { value: 'blue and white', label: '蓝白配色' },
                { value: 'warm tones', label: '暖色调' },
                { value: 'dark theme', label: '深色主题' },
                { value: 'gradient colors', label: '渐变色' },
                { value: 'monochrome', label: '单色系' },
                { value: 'vibrant and bold', label: '鲜艳大胆' },
                { value: 'pastel colors', label: '柔和色彩' },
            ]
        },
        {
            id: 'visual',
            name: '视觉元素',
            options: [
                { value: 'with icons', label: '含图标' },
                { value: 'with charts and graphs', label: '含图表' },
                { value: 'with high-quality images', label: '含高清图片' },
                { value: 'with illustrations', label: '含插画' },
                { value: 'data visualization', label: '数据可视化' },
                { value: 'minimal text', label: '少量文字' },
            ]
        },
        {
            id: 'typography',
            name: '字体排版',
            options: [
                { value: 'large bold headings', label: '大号粗体标题' },
                { value: 'clean sans-serif fonts', label: '简洁无衬线' },
                { value: 'elegant serif fonts', label: '优雅衬线' },
                { value: 'modern typography', label: '现代字体' },
                { value: 'hierarchical text', label: '层级分明' },
            ]
        },
        {
            id: 'feature',
            name: '文本语言',
            options: [
                { value: 'The text language is Chinese', label: '中文' },
                { value: 'The text language is English', label: '英文' }
            ]
        },
    ],
};
...
const PromptGeneratorTool: React.FC = () => {
    const [activeCategory, setActiveCategory] = useState<PromptCategory>('drawing');
    ...
    const handleModifierToggle = useCallback((groupId: string, value: string) => {
        setSelectedModifiers(prev => {
            const newModifiers = { ...prev };
            if (!newModifiers[groupId]) {
                newModifiers[groupId] = new Set();
            }
            const groupSet = new Set(newModifiers[groupId]);
    ...
    const handleGenerate = useCallback(() => {
        ...
        if (allModifiers.length > 0) {
            parts.push(allModifiers.join(', '));
        }

        const prompt = parts.join(', ');
        setGeneratedPrompt(prompt);
    }, [activeCategory, mainSubject, selectedModifiers]);
